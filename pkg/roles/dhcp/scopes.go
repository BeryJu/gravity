...
func (r *Role) findScopeForRequest(req *Request4) *Scope {
	...
	for scope := range r.scopes.Iter() {
		...
		clientIPMatchBits := scope.Value.match(req.ClientIPAddr)
		if clientIPMatchBits > -1 && clientIPMatchBits+clientIPBias > longestBits {
			req.log.Debug("selected scope based on client IP", zap.String("scope", scope.Value.Name))
			match = scope.Value
			longestBits = clientIPMatchBits + clientIPBias
		}
		// Check based on gateway IP (next highest priority)
		gatewayMatchBits := scope.Value.match(req.GatewayIPAddr)
		if gatewayMatchBits > -1 && gatewayMatchBits+dhcpRelayBias > longestBits {
			req.log.Debug("selected scope based on cidr match (gateway IP)", zap.String("scope", scope.Value.Name))
			match = scope.Value
		...
		localMatchBits := scope.Value.match(net.ParseIP(req.LocalIP()))
		if localMatchBits > -1 && localMatchBits > longestBits {
			req.log.Debug("selected scope based on cidr match (instance/interface IP)", zap.String("scope", scope.Value.Name))
			match = scope.Value
			longestBits = localMatchBits
		}
		// Fallback to default scope if we don't already have a match
		if match == nil && scope.Value.Default {
			req.log.Debug("selected scope based on default flag", zap.String("scope", scope.Value.Name))
			match = scope.Value
		}
	}
	if match != nil {
